...
static void write_colored(int fd, const char *buffer, size_t length)
{
    ...
    if (o == 3) {
        ...
        write_const(fd, COLOR_RESET);
        CFStringRef logMessage = CFStringCreateWithCStringNoCopy(kCFAllocatorDefault, memcpy((char *)buffer, buffer + space_offsets[2], length), kCFStringEncodingMacRoman, kCFAllocatorNull);
